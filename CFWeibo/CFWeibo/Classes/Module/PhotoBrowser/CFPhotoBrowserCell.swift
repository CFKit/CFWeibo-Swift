...
class CFPhotoBrowserCell: UICollectionViewCell {
    ...
    var newContentOffset: CGPoint = CGPoint.zero
    ...
    var url: URL? {
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(0 * Int64(NSEC_PER_SEC)) / Double(NSEC_PER_SEC)) {
                //  之前 imageView 没有指定大小
                ...
                self.imageView.sd_setImage(with: self.url, placeholderImage: self.placeholderImage, options: [SDWebImageOptions.retryFailed]) { (image, error, _, _) in
                    print("+++++++++++++++++")
    ...
    fileprivate func setScrollViewZoom() {
...
extension CFPhotoBrowserCell: UIScrollViewDelegate {
    ...
    /// 返回缩放过程中心点
    fileprivate func centerOfScrollViewContent(_ scrollView: UIScrollView) -> CGPoint {
